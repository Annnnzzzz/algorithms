...
struct ST {
    ...
    void dfs(int v, pair<int, int>& ans, int n) {
        ...
        for (auto& w : next[v]) {
            //cout << v << " " << w.second << " " << w.first << " " << l[w.second] << " " << r[w.second] << endl;
            if (l[w.second]<=n && n<r[w.second]) {
                
                achievable[v].insert(1);
                continue;
            }
            if (l[w.second] <= (2*n+1) && (2*n+1)<r[w.second]){
                achievable[v].insert(2);
                continue;
            }
            dfs(w.second, ans, n);
            achievable[v].insert(achievable[w.second].begin(), achievable[w.second].end());
           
        }
        if (achievable[v].size() == 2 && len_of_all[v] > ans.second) {
            int start = r[v] - len_of_all[v], end = r[v];
            //cout << "ans " << v << " " << r[v] << " " << l[v] << " " << len_of_all[v] << endl;

            if (hash_section(start, len_of_all[v], hash) == hash_section(2*n + 1 - len_of_all[v] - start, len_of_all[v], hash)) {
                //cout << "ans 2 " << v << " " << r[v] << " " << l[v] << " " << len_of_all[v] << endl;
                ans = { r[v] - len_of_all[v], len_of_all[v] };
            }
        }
    }
};
